...
\mathbb{P}(Y > `r round(VaR, 4)`) &= szs 
\end{aligned}
$$

```{r}
library(EnvStats)
funcion_mela <- function(malla){
  alpha <- malla[, 1]
  beta <- malla[, 2]
  
  media <- (alpha*beta)/(alpha - 1)
  cuantil <- qpareto(p = 0.05, location = beta, shape = alpha)
  
  err <- data.frame(alpha = alpha,
                    beta = beta,
                    Error_media = abs(media - mean_st), 
                    Error_cuantil = abs(cuantil - VaR))
  return(err)
}

mallita <- expand.grid(alpha = seq(0, 50, 0.01),
                       beta = seq(0, 50, 0.01))
```
