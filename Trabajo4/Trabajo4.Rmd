...
datos[datos$Payment == 0, 3] <- 1
```

\subsection{Ajuste de una Log-Normal}

Para ajustar una distribución Log-Normal a la variable Payment (pérdida en el portafolio), se utiliza la función `fitDist` del paquete `MASS` el cual estima los parámetros de la distribución de probabilidad mediante el método de máxima verosimilitud obteniéndose los siguientes parámetros estimados

```{r}
logn_fit <- MASS::fitdistr(datos$Payment, "log-normal")
mean_st <- logn_fit$estimate[1]
sd_st <- logn_fit$estimate[2]
```
...
El ajuste considerado no es muy adecuado debido a que infla mucho la probabilidad de los valores pequeños lo cual claramente difiere con lo observado con los datos (note la gran distancia existente entre la densidad estimada y el histograma observado), por otro lado, su capacidad de seguir la tendencia en la cola de la distribución no es despreciable, sin embargo, teniendo en cuenta que dichos valores son demasiado atípicos dicha utilidad no termina siendo satisfactoria si el precio a pagar es tener poca precisión en el "rango normal" de los datos.
...
Si se tiene un portafolio de 20 pólizas, con una tasa de reclamos por póliza de 0 = 0.015 y un cargo de seguridad $\lambda = 0.05$ en la prima, las reservas necesarias al inicio de un periodo (año) UV para que la aseguradora pueda honrar los contratos con una confianza del 95 \% deben cumplir: De nuevo si $X$ corresponde a la perdida en el portafolio
...
$$
f(x) = \frac{\alpha}{\beta} \left(\frac{\beta}{x}\right)^{1 + \alpha}, x > \beta
$$
